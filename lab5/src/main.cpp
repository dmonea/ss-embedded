...
const unsigned long STREAM_INTERVAL = 20; // ms
...
void callback(char *topic, const byte *payload, unsigned int length) {
  ...
  if (String(topic) == TOPIC_COMMAND) {
    if (message == "CAPTURE") {
      ...
      Serial.println("=> Action: Flash Toggle!");
    } else {
...
void loop() {
  ...
  if (now - last_heartbeat > 5000) {
    Serial.printf(
        "[heartbeat] millis=%lu connected=%d streaming=%d free_heap=%u\n", now,
        client.connected(), streaming, ESP.getFreeHeap());
    last_heartbeat = now;
  }
